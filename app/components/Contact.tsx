...
export default function Contact() {
  return (
    ...
    >
      ...
      />
      <h2 className="text-xl md:text-3xl font-serif tracking-wide mb-6 px-6 md:px-10">
        Find Me Here
      </h2>
      <motion.div
      ...
      <div className=" flex items-center justify-between gap-5 lg:flex-row flex-col md:px-10 lg:px-10">
        <div className="flex flex-row justify-start lg:gap-3 gap-0.5 md:gap-4 py-2 md:py-4">
          ...
          />
          <ContactCard
            icon={<GithubIcon size={24} className="text-muted-foreground" />}
        ...
        >
          ...
          <span className="border-b  mt-0.5 border-b-green-600 border-off border-dashed">
            Vineet Jadhav
          </span>
        </motion.div>
      </div>
...
export function ContactCard({
  ...
  return (
    <a href={href} target="_blank" rel="noopener noreferrer">
      <Button
        size="icon-sm"
        className={`
    bg-transparent border border-dashed cursor-pointer
  ...
  `}
      >
        {icon}
        <p className="text-xs md:text-sm font-inter tracking-wide text-muted-foreground ">
          {label}
        </p>
      </Button>
    </a>
